-- Complete fix for signup data routing issues
...
-- Drop tables with inconsistent naming
...
CREATE TABLE IF NOT EXISTS profiles (
    ...
    phone TEXT,
    organization_id UUID, -- Will reference organizations(id)
...
DROP POLICY IF EXISTS "team_invites_org_manage" ON team_invites;
...
CREATE POLICY "team_invites_org_manage" ON team_invites 
FOR ALL USING (
    organization_id IN (
        SELECT id FROM organizations WHERE owner_id = auth.uid()
    )
...
CREATE POLICY "team_invites_view_own" ON team_invites 
FOR SELECT USING (email = auth.jwt() ->> 'email');

-- =============================================================================
-- STEP 4: Create the corrected user registration function
-- =============================================================================
...
SELECT 'Signup data routing fix completed successfully!' as message;
...
WHERE table_name IN ('profiles', 'organizations', 'parents', 'team_members', 'team_invites')
...
WHERE tc.table_name IN ('profiles', 'organizations', 'parents', 'team_members', 'team_invites')
