...
const ServiceBookingForm = ({ onClose, onSuccess, organizationId, userId, initialService = null, bookingToEdit = null, userRole }) => {
  ...
  const loadInitialData = async () => {
    ...
    if (orgIdToLoad) {
      const { data: orgData } = await serviceManagementService.getOrganizationById(orgIdToLoad);
      setSelectedOrg(orgData);
      const { data: servicesData } = await serviceManagementService.getOrganizationServices(orgIdToLoad);
  ...
  const checkAvailability = async () => {
    ...
    const { data } = await bookingService.getBookedSlots(selectedOrg.id, formData.scheduledDate);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = bookingToEdit
        ? await bookingService.updateServiceBooking(bookingToEdit.id, bookingData)
        : await bookingService.createServiceBooking(bookingData);

      if (result.error) setError(result.error);
      else {
        const action = bookingToEdit ? 'update' : 'create';
        await auditService.logAction({ bookingId: result.data.id, action, newStatus: result.data.status, actorId: userId, actorRole: userRole });
  ...
  const handleStatusChange = async (newStatus, skipConfirm = false) => {
    ...
    if (!error) {
      await auditService.logAction({ bookingId: bookingToEdit.id, action: 'status_update', oldStatus: bookingToEdit.status, newStatus, actorId: userId, actorRole: userRole });
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-white border-b border-gray-50 p-6 flex justify-between items-center z-10">
          <div className="flex space-x-6">
            <button onClick={() => setActiveTab('details')} className={`text-lg font-bold transition-colors ${activeTab === 'details' ? 'text-navy' : 'text-gray-400'}`}>
              Details
            </button>
            {bookingToEdit && (
              <button onClick={() => setActiveTab('audit')} className={`text-lg font-bold transition-colors flex items-center ${activeTab === 'audit' ? 'text-navy' : 'text-gray-400'}`}>
          ...
          {activeTab === 'details' ? (
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <select name="serviceId" value={formData.serviceId} onChange={(e) => setFormData({ ...formData, serviceId: e.target.value })} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500 disabled:opacity-50" required disabled={isReadOnly}>
              ...
              <div className={`p-6 rounded-2xl border ${isReadOnly ? 'bg-gray-50 border-gray-100' : 'bg-teal-50/50 border-teal-100'}`}>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="date" name="scheduledDate" value={formData.scheduledDate} onChange={(e) => setFormData({ ...formData, scheduledDate: e.target.value })} className="w-full p-3 bg-white border border-gray-100 rounded-xl focus:ring-2 focus:ring-teal-500" required disabled={isReadOnly} />
                  </div>
                  <div>
                    ...
                    {isReadOnly ? (
                      ...
                      <TimeSlotPicker selectedTime={formData.scheduledTime} onSelect={(time) => setFormData(prev => ({ ...prev, scheduledTime: time }))} bookedSlots={bookedSlots} loading={loadingSlots} />
                    )}
                  </div>
                </div>
              </div>
