...
const ListingBrowse = () => {
  ...
  }, [listings, filters.lat, filters.lng, filters.radius]);
  ...
  const filterListingsByRadius = () => {
    if (!filters.lat || !filters.lng || filters.radius === 0) {
      // No location filter or "Anywhere" selected - show all
    ...
    const withoutCoords = listings.filter(listing => {
  ...
  const clearLocationFilter = () => {
    const newFilters = { location: '', lat: null, lng: null, radius: 20 };
    setFilters(newFilters);
    setSearchParams({});
  };
      ...
      <div className="space-y-8">
        ...
        {filters.location && (
          <div className="flex items-center justify-center gap-2 text-sm">
            <span className="text-gray-500">Showing results within</span>
          ...
          <div className="text-center py-24 bg-white rounded-[3rem] border-2 border-dashed border-gray-100 max-w-2xl mx-auto shadow-sm">
            ...
            <p className="text-gray-500 max-w-sm mx-auto">
              {filters.location 
                ...
                : 'Try searching for a location to find safety experts near you.'}
            </p>
            {filters.location && (
              <button 
