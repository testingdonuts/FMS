...
const { 
  FiSave, FiX, FiPlus, FiMapPin, FiArrowLeft, FiArrowRight, 
  FiInfo, FiImage, FiClock, FiLoader, FiCheck, 
  FiFacebook, FiInstagram, FiTwitter, FiLinkedin, FiTrash2, 
  FiPlusCircle, FiUploadCloud, FiCamera, FiTruck, FiAlertCircle, 
  FiCreditCard, FiStar, FiZap, FiEye, FiEyeOff
} = FiIcons;
...
const ListingForm = ({ listing = null, onSave, onCancel, isLoading = false }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    opening_hours: daysOfWeek.reduce((acc, day) => ({ ...acc, [day]: { open: '09:00', close: '17:00', closed: false } }), {}),
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="px-8 pt-8 pb-4 border-b border-gray-100 flex-shrink-0 bg-white">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <h2 className="text-2xl font-bold text-navy tracking-tight">{listing ? 'Edit Listing' : 'Create New Listing'}</h2>
            <div className="flex space-x-1 bg-gray-100/50 p-1 rounded-2xl w-fit">
              {tabs.map((tab) => (
                <button 
                  ...
                  className={`flex items-center space-x-1.5 px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-white text-navy shadow-md' : 'text-gray-400 hover:text-gray-600'}`} 
                >
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 custom-scrollbar bg-white">
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.2 }} className="max-w-3xl mx-auto py-4" >
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className={`p-6 rounded-3xl border-2 transition-all flex items-center justify-between ${formData.status === 'published' ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${formData.status === 'published' ? 'bg-green-500' : 'bg-gray-300'}`}
                      >
                    ...
                    <div className={`p-6 rounded-3xl border-2 transition-all flex items-center justify-between ${formData.is_featured ? 'bg-soft-yellow/10 border-soft-yellow shadow-lg shadow-soft-yellow/5' : 'bg-gray-50 border-gray-100'}`}>
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${formData.is_featured ? 'bg-soft-yellow' : 'bg-gray-300'} ${!canBeFeatured && 'opacity-30 cursor-not-allowed'}`}
                      >
                ...
                <div className="space-y-6">
                  <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100 flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="bg-blue-600 p-2 rounded-xl text-white"><SafeIcon icon={FiTruck} /></div>
                      <div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-12 p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500" 
                      />
                    </div>
                  </div>
                      ...
                      <div className="w-24 h-24 bg-white rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden relative group">
                        {formData.logo_url ? (
                          <>
                            ...
                            <button type="button" onClick={() => setFormData(p => ({ ...p, logo_url: '' }))} className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white transition-opacity"><SafeIcon icon={FiTrash2} /></button>
                          </>
                        ) : (
                          uploadingLogo ? <SafeIcon icon={FiLoader} className="animate-spin text-blue-600" /> : <SafeIcon icon={FiCamera} className="text-2xl text-gray-300" />
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {formData.gallery_urls && formData.gallery_urls.map((url, idx) => (
                        <div key={idx} className="aspect-square bg-white rounded-xl border border-gray-100 overflow-hidden relative group">
                          ...
                          <button type="button" onClick={() => setFormData(p => ({...p, gallery_urls: p.gallery_urls.filter((_, i) => i !== idx)}))} className="absolute top-1 right-1 p-1.5 bg-red-500 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"><SafeIcon icon={FiX} /></button>
                        </div>
                      ))}
                    </div>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50/80 flex justify-between items-center flex-shrink-0">
          <button 
            ...
            className={`flex items-center space-x-2 px-6 py-3 font-bold rounded-2xl transition-all ${activeTab === 'basic' ? 'text-gray-300 opacity-50' : 'text-gray-600 hover:bg-gray-200'}`} 
          >
