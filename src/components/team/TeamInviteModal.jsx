...
const TeamInviteModal=({isOpen,onClose,onSendInvite,organizationId,currentUserId,onUpgrade})=> {
  const [formData,setFormData]=useState({email: '',role: 'technician',});
  ...
  const handleSubmit=async (e)=> {
    ...
    try {
      ...
      if (result?.error) {
        console.error('[TeamInviteModal] Invitation error:', result.error);
      ...
      console.error('[TeamInviteModal] Unexpected error:', err);
  ...
  const roleOptions=[
    ...
    {value: 'staff',label: 'Staff',description: 'General access for non-technical roles.'}
  ];
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 ${errors.email ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}
                ...
                <div className="space-y-3">
                  ...
                  {roleOptions.map((role)=> (
                    <label
                      key={role.value}
                      className={`flex items-start gap-3 p-4 border-2 rounded-2xl cursor-pointer transition-all ${formData.role===role.value ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-blue-200'}`}
                    >
                      <input
                        type="radio"
                        name="role"
