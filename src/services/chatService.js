import { supabase } from '../lib/supabase';
    ...
    export const chatService = {
      ...
      async getMessages(userId, otherUserId, context = {}) {
        ...
        } else if (context.equipmentId) {
          query = query.eq('equipment_id', context.equipmentId);
        } else {
          // No context = general messages only (all context fields should be null)
          query = query.is('booking_id', null).is('service_id', null).is('equipment_id', null);
        }
    ...
    if (error) {
      console.error("[chatService] Error fetching conversations via RPC:", error);
      return { data: [], error: error.message };
    }

    console.log('[chatService] Raw RPC response:', data);
    ...
    console.log('[chatService] Formatted conversations:', formattedConversations);
