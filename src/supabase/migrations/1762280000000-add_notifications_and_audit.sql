/*
    ...
    1. New Tables
        ...
        - `booking_audit_logs`: Stores a history of all actions performed on a booking.
    ...
    */
    ...
    CREATE POLICY "Viewable by booking participants" ON booking_audit_logs
        FOR SELECT TO authenticated
        USING (
            EXISTS (
                SELECT 1
                FROM public.service_bookings sb
                WHERE sb.id = booking_id AND (sb.parent_id = auth.uid() OR sb.org_id = get_user_organization_id(auth.uid()))
            )
        );