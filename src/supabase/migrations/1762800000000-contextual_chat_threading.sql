/*
    ...
    1. Changes
        - Add `booking_id` (uuid) to `chat_messages`
        - Add `service_id` (uuid) to `chat_messages`
        - Add `equipment_id` (uuid) to `chat_messages`
    ...
    */
    ...
    CREATE INDEX IF NOT EXISTS idx_chat_service ON public.chat_messages(service_id);