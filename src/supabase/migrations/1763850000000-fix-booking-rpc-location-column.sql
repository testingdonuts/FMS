/*
# Fix Booking RPC Functions - Remove location_id Reference

This migration addresses a "column sb.location_id does not exist" error that occurs when fetching bookings.

## 1. Problem
The `get_organization_bookings` and `get_parent_bookings` RPC functions are trying to SELECT a `location_id` column from the `service_bookings` table, but this column doesn't actually exist in that table yet (it was added to other tables but not to service_bookings in the Teams plan migration).

## 2. Solution
This migration updates both RPC functions to remove the `location_id` column from their return signature and SELECT statements, resolving the query error.

## 3. Impact
This allows both parents and organizations to successfully fetch their booking data without the column mismatch error.
*/
